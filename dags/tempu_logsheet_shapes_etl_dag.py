...
def clean_dataframe(df):
    ...
    cols_to_drop = df.columns[df.columns.str.contains('unnamed', case=False)].tolist()
    if cols_to_drop:
        logging.info(f"Dropping the following unnamed columns: {cols_to_drop}")
    else:
        logging.info("No unnamed columns found to drop in this file.")
    ...
    rename_map = {
        # Previous variations / legacy columns
        ...
        'data_extracted__synched_date': 'data_extracted_date',
        'data_extracted__synched_time': 'data_extracted_time',
        'device_provided_date_device_given': 'device_provided_date',
        's.no': 's_no',
        'intervention_arm__intervention_contol': 'intervention_arm',
        'intervention_arm__intervention_control': 'intervention_arm',
    ...
    # 1. Apply Business Rule: IF remarks is not empty AND time is missing -> fill '00:00'
    # Create a mask for rows where 'remarks' is present (not NaN and not just empty spaces)
    has_remarks = df['remarks'].notna() & (df['remarks'].astype(str).str.strip() != "")

    time_cols = ['device_provided_time', 'device_removed_time', 'data_extracted_time']
    for col in time_cols:
        # Use .loc to fill '00:00' only where the condition is met and the time is currently missing
        df.loc[has_remarks & df[col].isna(), col] = '00:00'

    # 2. Clean Dates: Extract ONLY the YYYY-MM-DD string
    clean_prov_date = pd.to_datetime(df['device_provided_date'], format='mixed', errors='coerce').dt.strftime(
        '%Y-%m-%d')
    clean_rem_date = pd.to_datetime(df['device_removed_date'], format='mixed', errors='coerce').dt.strftime('%Y-%m-%d')
    clean_ext_date = pd.to_datetime(df['data_extracted_date'], format='mixed', errors='coerce').dt.strftime('%Y-%m-%d')

    # 3. Clean Times Safely: Prevent the literal string "nan" bug
    def clean_time_safely(series):
        # .astype(str) turns NaN into "nan". We replace it with None so string addition yields NaN instead of "Date nan"
        return series.astype(str).replace(['nan', 'None', '<NA>'], None).str.split('.').str[0]

    clean_prov_time = clean_time_safely(df['device_provided_time'])
    clean_rem_time = clean_time_safely(df['device_removed_time'])
    clean_ext_time = clean_time_safely(df['data_extracted_time'])

    # 4. Combine and convert to final datetime
    # If time is None, Date + ' ' + None = NaN. pd.to_datetime turns NaN into NaT gracefully!
    df['device_provided_datetime'] = pd.to_datetime(clean_prov_date + ' ' + clean_prov_time, format='mixed',
                                                    errors='raise')
    df['device_removed_datetime'] = pd.to_datetime(clean_rem_date + ' ' + clean_rem_time, format='mixed',
                                                   errors='raise')
    df['data_extracted_datetime'] = pd.to_datetime(clean_ext_date + ' ' + clean_ext_time, format='mixed',
                                                   errors='raise')

    # 5. Drop original date and time columns
    df.drop(columns=[
    ...
    # 6. Format to string with milliseconds
    ...
    for file in files:
        ...
        try:
            ...
            try:
                ...
                # Check if first row has valid headers (e.g., contains any 'Region' or 'S.No' or 'Remarks')
                # if not any(df.columns.str.contains('device_id', case=False)):
                #     logging.warning(f"First row does not contain expected headers in {file}. Columns found: {df.columns.tolist()}")
                #     raise ValueError("First row does not contain valid headers, likely due to extra header rows. Triggering fallback.")

                # Columns after initial read
                logging.info(f"Columns after initial read: {df.columns.tolist()}")
            except Exception as e1:
                logging.warning(f"Standard read failed, retrying with fallback: {e1}")
            ...
            try:
                hook.send(text=f":red_circle: Corrupt Excel file skipped: {file}")
            except:
                logging.warning("Slack alert failed (rate limit likely)")
...
def load(**context):
    ...
    for transformed_path in transformed_files:
        ...
        # Df Columns
        print(f"Columns in {transformed_path} before loading: {df.columns.tolist()}")
        logging.info(f"[LOAD] Loading data from: {transformed_path}")
        print(f"DataFrame head before loading:\n{df.head(5)}")
        ...
        for _, row in df.iterrows():
            try:
                cursor.execute("""
                               INSERT INTO tempu_data_logsheet_shapes (region, cluster, intervention_arm,
                                                                       intervention_phase, device_id, devices_locations,
                                                                       participant_id,
                                                                       device_provided_datetime,
                                                                       device_removed_datetime,
                                                                       final_use, data_extracted_datetime,
                                                                       data_filename,
                                                                       remarks, filename)
                               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                               """,
                               *[
                                   ...
                                   for x in [
                                       row['region'],
                                       row['cluster'],
                                       row['intervention_arm'],
                                       row['intervention_phase'],
                                       row['device_id'],
                                       row['devices_locations'],
                                       row['participant_id'],
