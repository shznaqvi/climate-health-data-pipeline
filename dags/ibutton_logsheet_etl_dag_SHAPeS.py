...
FOLDER_PATH = '/opt/airflow/dags/csv/ibutton_logsheets/SHAPES'
...
def clean_dataframe(df):
    ...
    rename_map = {
        "participant": "region",  # or maybe keep separate if 'region' is not here
        "device_serial": "device_id",
        "participant_id": "participant_id",
        "date_of_device_issue": "device_provided_date",
        ...
        "time_of_device_placment": "device_provided_time",
        # "date_of_extract": "data_extracted_date",
        # "data_extraction_date": "data_extracted_date",
        # "data_extraction_time": "data_extracted_time",
        "device_removed_date_device_taken_back": "device_removed_date",
        "return_date": "device_removed_date",
        "time_of_device_removed": "device_removed_time",
        "date_of_device_removed": "device_removed_date",
    ...
    invalid_zeros = ['0']
    time_columns = ['device_provided_time', 'device_removed_time']
    print(f"Df Columns list: {df.columns.tolist()}")

    # if 'data_extracted_date' not in df.columns:
    #     df['data_extracted_date'] = df['device_removed_date'].copy()
...
def transform(**kwargs):
    ...
    for file in files:
        ...
        try:
            possible_sheets = ["iButton Data Log", "Data extract log sheet", "Data Logsheet", "i-button log",
                               "Ibutton Data Log"]

            df = pd.ExcelFile(file)
            sheet_to_use = next((s for s in possible_sheets if s in df.sheet_names), None)

            if not sheet_to_use:
                raise ValueError(f"No valid sheet found in {file}. Available sheets: {df.sheet_names}")
        ...
        # Add region column only if it does not exist
        ...
        if df.empty:
            message = f":red_circle: No data after transformation for all iButton Logsheet Files: {file}"
            hook.send(text=message)
...
# DAG Definition
dag = DAG(
    "ibutton_logsheet_etl_dag_SHAPeS",
...
wait_for_file_task = FileSensor(
    ...
    fs_conn_id="fs_conn_ibutton_logsheets_SHAPES",
    filepath="*.xlsx",
    poke_interval=30,
    timeout=600,
    mode="poke",
