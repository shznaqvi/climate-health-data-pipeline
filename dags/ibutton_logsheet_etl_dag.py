...
pd.set_option('display.max_columns', None)  # Show all columns in logs
pd.set_option('display.width', 1000)  # Set display width for better readability
pd.set_option('display.max_rows', 100)  # Show more rows in logs
pd.set_option('display.float_format', lambda x: f'{x:.3f}')  # Format floats to 3 decimal places
...
def extract(**kwargs):
    ...
    # file_path = '/opt/airflow/dags/csv/ibutton_logsheets/Copy of Ibuttions Log Sheet_Suhrab Solangi_30042025_HN.xlsx'
    ...
    if not files:
        message = ":warning: No ibutton Excel files found."
...
def transform(**kwargs):
    ...
    possible_sheets = ["i-button log", "iButton Data Log", "iButton Log", "I-Button Log", "Ibutton Log",
                       "Ibutton Data Log"]
    ...
    for fname in filenames:
        ...
        try:
            ...
            # print(f'Unique values in participant_id column:\n{df["participant_id"].unique()}')
            # print(f'Number of NaN values in participant_id column: {df["participant_id"].isna().sum()}')
            # print(f'Value counts in participant_id column:\n{df["participant_id"].value_counts(dropna=False)}')
            # print(f'Data types in participant_id column:\n{df["participant_id"].apply(type).value_counts()}')
            df['filename'] = fname
...
def load(**kwargs):
    ...
    for csv in csv_files:
        ...
        # first row from df
        print(f"First row of DataFrame:\n{df.head(1)}")

        # print(df['participant_id'].isna().sum())
        # print(df['participant_id'].value_counts(dropna=False).head(20))
        # print(df['participant_id'].apply(type).value_counts())
        # print(f'Unique values in participant_id column:\n{df["participant_id"].unique()}')
        # print(f'Number of NaN values in participant_id column: {df["participant_id"].isna().sum()}')
        # print(f'Value counts in participant_id column:\n{df["participant_id"].value_counts(dropna=False)}')
        # print(f'Data types in participant_id column:\n{df["participant_id"].apply(type).value_counts()}')
        logging.info(f"[LOAD] Loading data from: {csv}")

        print(f"df.head() before loading to DB:\n{df.head()}")
        records = [
            (
                row.region,
                row.device_id,
...
wait_for_file = FileSensor(
    ...
    fs_conn_id="fs_conn_ibutton_logsheets_abv",  # Make sure this connection exists in Airflow
