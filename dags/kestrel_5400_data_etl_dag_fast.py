...
def fix_datetime_format(df):
    ...
    # logging.info(f"Fixing DateTime format...:{df['DateTime']}")
    ...
    # Re-format DateTime to '%Y-%m-%d %H:%M:%S'docker compose down
    df['DateTime'] = pd.to_datetime(
        df['DateTime'],
        format='%Y-%m-%d %I:%M:%S %p'  # 2025-10-06 04:25:00 am
    )
...
def transform(**kwargs):
    ...
    for file in files:
        ...
        if 'serial number' not in metadata:
            logging.error(f"Serial number missing in file {file}, skipping.")
            continue

        # Read CSV skipping metadata rows
        df = pd.read_csv(file, skiprows=3, dtype=str)
        df = df.iloc[1:].reset_index(drop=True)  # drop repeated header if exists

        # Add metadata columns
        df['Device Name'] = metadata.get('device name')
        df['Device Model'] = metadata.get('device model')
        df['Serial Number'] = metadata.get('serial number')
        df['File Name'] = os.path.basename(file)
        ...
        df = df.replace({np.nan: None})  # Prepare for DB load

        # Write cleaned CSV to temp folder
        temp_file = os.path.join(FOLDER_PATH, TEMP_FOLDER, os.path.basename(file))
        os.makedirs(os.path.dirname(temp_file), exist_ok=True)
        df.to_csv(temp_file, index=False)

        transformed_files.append(temp_file)
        logging.info(f"Transformed {df.shape[0]} rows from {file}")

    # Push only file paths to XCom
...
def load(**kwargs):
    ...
    logging.info(f"🎯 Data load complete: {fileCount} files, {rowcount} total rows in {elapsed}s.")


# Define the DAG
dag = DAG(
    'kestrel_5400_data_etl_dag_fast',
