...
def transform(**kwargs):
    ...
    for file in files:
        ...
        try:
            ...
            # If UTF-8 fails, we log it and try Latin-1
            logging.warning(f"UTF-8 failed for {file}. Error: {e}. Trying Latin-1 fallback...")

            try:
                # Attempt 2: Latin-1 (this almost never throws a DecodeError)
                with open(file, 'r', encoding='latin-1') as f:
                    lines = f.readlines()
                logging.info(f"Successfully recovered {file} using Latin-1.")
                logging.info(f"Lines read from {file}:\n {lines}")

            except Exception as e2:
                # If even the fallback fails, then we notify and crash
                message = f"❌ CRITICAL: Both UTF-8 and Latin-1 failed for {file}. Error: {e2}"
                logging.error(message)
                hook.send_text(message)
                raise  # This finally marks the Airflow task as Failed

            # Latin-1 (ISO-8859-1) can read almost any byte sequence without crashing
            # with open(file, 'r', encoding='latin-1') as f:
            #     lines = f.readlines()
        ...
        metadata = {
            'DeviceID': get_value('ID'),
            'LogInterval': get_value('Log Interval'),
            'StartMode': get_value('Start Mode'),
            'StopMode': get_value('Stop Mode'),
            'TripDuration': get_value('Trip Length'),
            'PointsLogged': get_value('Number of Points'),
            'TimeFormatNote': next((l.strip() for l in lines if "All times shown" in l), None)
        }

        header_index = next((i for i, line in enumerate(lines) if line.strip().startswith('Date')), None)
        try:
            df = pd.read_csv(file, skiprows=header_index).iloc[1:].reset_index(drop=True)
            # Attach metadata
            for k, v in metadata.items():
                df[k] = v
            df['File Name'] = os.path.basename(file)
            # logging.info(f"DF.columns after reading: {df.columns.tolist()}")
            # Clean
            df = clean_df(df)
            # logging.info(f"DataFrame shape after reading: {df['TemperatureC'].head()}")

            # Write cleaned CSV to temp folder for load step
            temp_file = os.path.join(FOLDER_PATH, TEMP_FOLDER, os.path.basename(file))
            ...
            message = f"File {file} is empty or has no data rows after header."
            logging.error(message)
            hook.send(text=message)
            continue
        except Exception as e:
            move_file_to_done(os.path.basename(file), "InValid")
            message = f"File {file} is In-Valid or DATE & TIME is wrong."
            logging.error(message)
            hook.send(text=message)
            continue
    ...
    for file in processed_files:
        ...
        data_tuples = [
            tuple(row.get(col, None) for col in [
                'LogDateTime', 'TemperatureC', 'HumidityRH', 'DeviceID', 'LogInterval',
                'StartMode', 'StopMode', 'TripDuration', 'PointsLogged', 'TimeFormatNote', 'filename'
            ])
